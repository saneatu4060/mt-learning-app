...
const ModeSelect = () => {
    ...
    const handleModeSelectam = () => {
        router.push(`/exam/${categoryId}/${data?.am}`)
    };
    const handleModeSelectpm = () => {
        router.push(`/exam/${categoryId}/${data?.pm}`)
    };
        ...
        <div className="container mx-auto p-4">
            <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-xl p-8 max-w-3xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    {(parts[3] === 'exam') ? (null) : (<Link href="/exam" className="text-gray-600 hover:text-gray-900 text-sm">
                        ← 戻る
                    </Link>)}
                </div>
